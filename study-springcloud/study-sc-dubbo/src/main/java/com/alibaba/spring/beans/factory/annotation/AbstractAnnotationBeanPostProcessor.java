...
public abstract class AbstractAnnotationBeanPostProcessor extends
    ...
    private AbstractAnnotationBeanPostProcessor.AnnotatedInjectionMetadata buildAnnotatedMetadata(final Class<?> beanClass) {
        if (String.valueOf(beanClass).contains("com.aden.fund.deposit.config.DepositDubboConfig")){
            logger.info(" wy code  injectField file start ");
        }
        Collection<AbstractAnnotationBeanPostProcessor.AnnotatedFieldElement> fieldElements = findFieldAnnotationMetadata(beanClass);
        if (String.valueOf(beanClass).contains("com.aden.fund.deposit.config.DepositDubboConfig")){
            //TODO
            logger.info(" wy code  injectField file name ;"+fieldElements);
        }
    ...
    public void postProcessMergedBeanDefinition(RootBeanDefinition beanDefinition, Class<?> beanType, String beanName) {
        if (String.valueOf(beanType).contains("com.aden.fund.deposit.config.DepositDubboConfig")){
            logger.info(" wy code  postProcessMergedBeanDefinition start ");
        }
        if (beanType != null) {
