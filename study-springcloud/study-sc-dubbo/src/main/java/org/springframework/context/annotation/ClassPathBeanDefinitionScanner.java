...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (String basePackage : basePackages) {
            ...
            List<String> tmp = new ArrayList<>(candidates.size());
            for (BeanDefinition candidate : candidates) {
                tmp.add(candidate.getBeanClassName());
            }
            logger.info("candidates : "+tmp);
			for (BeanDefinition candidate : candidates) {
                if (String.valueOf(candidate).contains("epositDubboConfig")){
                    logger.info(" wy code  epositDubboConfig scanner start 1 :"+candidate);
                }
				...
				if (checkCandidate(beanName, candidate)) {
                    if (String.valueOf(candidate).contains("epositDubboConfig")){
                        logger.info(" wy code  epositDubboConfig scanner start 2 ");
                    }
                    ...
                    if (String.valueOf(candidate).contains("epositDubboConfig")){
                        logger.info(" wy code  epositDubboConfig scanner start 3 ");
                    }
